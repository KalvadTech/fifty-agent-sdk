...
|--------|----------|----------------|----------|-----------------------|
| **OPEN** | Consumer B, in **production** code (not just tests) | `AgentRunner._state` (`src/fifty_agent_sdk/runner.py`) | 2026-07-30 | **None exists today.** There is no public accessor for a runner's state store, and the consumer needs the *exact instance* — shared engine **and** shared per-session lock registry — so constructing a second store over the same engine is not an equivalent workaround. Same shape as BR-010: papering over a missing public surface. Tracked as **BR-011**; a read-only `AgentRunner.state` property is the likely fix. |
